...
export function setupSocketHandlers(io: Server): void {
  io.on('connection', (socket: Socket) => {
    ...
    socket.on('join_room', (data: { code: string; userId: string; username: string }) => {
      ...
      const existingIdx = room.players.findIndex(p => p.userId === data.userId);
      if (existingIdx >= 0) {
    ...
    socket.on('start_match', async (data: { code: string }) => {
      ...
      if (room.players.length < 2) {
        socket.emit('error', { message: 'São necessários pelo menos 2 jogadores' });
        return;
      }
    ...
    socket.on('player_input', (data: {
      code: string;
      userId: string;
      inputSequence: GameColor[];
    }) => {
      const room = activeRooms.get(data.code);
      if (!room || room.status !== 'input') return;

      const isCorrect = data.inputSequence.every(
        (color, i) => color === room.sequence[i]
      ...
      const alivePlayers = room.players.filter(p => p.isAlive);
      const currentIdx = alivePlayers.findIndex(p => p.userId === data.userId);
