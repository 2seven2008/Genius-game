...
export function MultiplayerScreen() {
        ...
        setRoom((currentRoom) => {
          if (currentRoom) {
            showSequenceAnimation(
              seq,
              currentRoom.code,
              socket.id === currentRoom.hostSocketId,
            );
          }
          return currentRoom;
        });
  ...
  const handleCreateRoom = async () => {
    if (!user) {
      toast.error("Faça login para criar salas");
      return;
    }
    setIsCreating(true);
    try {
      ...
      <AnimatePresence mode="wait">
        {screen === "lobby" && (
          ...
          >
            <div className="flex items-center gap-3 mb-6">
              ...
              <h1 className="font-display font-black text-neon-blue text-2xl tracking-wider">
                MULTIPLAYER
              </h1>
            </div>
              ...
              <Card>
                ...
                <div className="flex flex-col gap-2">
                  {displayedRooms.map((r) => (
                    <div
                      key={r.code}
                      className="flex items-center justify-between bg-dark-base rounded-lg px-3 py-2"
                    >
                      <div>
                        <span className="text-white font-display font-bold text-sm">
                          Modo: Versus
                        </span>
                        <span className="text-zinc-500 text-xs font-body ml-2">
                          Host: {r.host}
                        </span>
                      </div>
                      <NeonButton
                ...
                {publicRooms.length > 4 && (
                  ...
                  >
                    <ChevronLeft
                      className={`w-4 h-4 transition-transform ${showMore ? "rotate-90" : "-rotate-90"}`}
                    />
            ...
            <Card className="mb-4 p-4">
              ...
              <div className="flex flex-col gap-2">
                {room.players.map((p) => (
                  <div
                    key={p.userId}
                    className="flex items-center gap-3 bg-dark-base rounded-lg px-3 py-2"
                  >
                ...
                {Array.from({
                  ...
                  <div
                    key={i}
                    className="flex items-center gap-3 bg-dark-base/50 rounded-lg px-3 py-2 border border-dashed border-dark-border"
                  >
                    <div className="w-8 h-8 rounded-full bg-dark-muted/50 flex items-center justify-center">
                      <Users className="w-4 h-4 text-zinc-700" />
                    </div>
                    <span className="text-zinc-600 font-body text-sm">
                      Aguardando...
                    </span>
                  </div>
                ))}
        ...
        {screen === "game" && room && (
          ...
          >
            ...
            <ScoreDisplay
              score={
                room.players.find((p) => p.userId === user?.id)?.score ?? 0
              }
              label={`NÍVEL ${gameLevel}`}
            />
            ...
            <div className="text-center">
              {showingSequence && (
                <p className="text-zinc-400 font-body text-sm animate-pulse">
                  Observe a sequência...
                </p>
              )}
              {isMyTurn && !showingSequence && (
                <p className="text-neon-purple font-display font-bold animate-pulse">
                  SUA VEZ!
                </p>
              )}
              {!isMyTurn && !showingSequence && (
          ...
          >
            ...
            <div className="text-center">
              {gameResult.winner?.userId === user?.id ? (
                <>
                  <p className="text-neon-yellow font-display font-black text-5xl">
                    🏆
                  </p>
                  <p className="text-neon-yellow font-display font-black text-3xl mt-2">
                    VITÓRIA!
                  </p>
                </>
              ) : (
                <>
                  ...
                  <p className="text-zinc-400 font-display font-bold text-2xl mt-2">
                    DERROTA
                  </p>
                </>
              )}
            ...
            <Card className="w-full p-4">
              <p className="font-display font-bold text-white mb-3 text-sm">
                Placar Final
              </p>
              {gameResult.players
                ...
                .map((p, i) => (
                  ...
                  >
                    <div className="flex items-center gap-2">
                      <span className="text-zinc-500 font-display text-sm w-4">
                        {i + 1}°
                      </span>
            ...
            <div className="flex gap-3 w-full">
              <NeonButton variant="gray" fullWidth onClick={handleLeave}>
                Sair
              </NeonButton>
              {room && (
                <NeonButton
